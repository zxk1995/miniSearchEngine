...
void Cache::writeToFile(const string & cachePath)
{
	...
	ofstream os("/home/zxk666/miniSearchEngine/online/data/cache.dat");

	if(!os.good())
	{
		cout << "cache file open error" << endl;
		exit(EXIT_FAILURE);
	}
	Node* cur = _tail;
...
void Cache::readFromFile(const string & cachePath)
{
	...
	ifstream is("/home/zxk666/miniSearchEngine/online/data/cache.dat");

	if(!is.good())
	{
		cout << "cache file open errror" << endl;
		exit(EXIT_FAILURE);
	}
	string key;
	string value;
